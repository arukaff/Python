...
if response.status_code == 200:
    ...
    print(response.text)
# Здравствуйте Алексей! Код выполняет поиск заведений по категориям в заданном городе, используя Foursquare API, и выводит название, адрес и рейтинг заведений. 
# Замечание: в параметре "sort" лучше использовать sort='RELEVANCE', так как "RATING" не поддерживается этим API. 
# Также для повышения надежности добавьте проверку на наличие данных перед выводом (например, place.get("location", {}).get("address")) и укажите значение категории по умолчанию, если введено неверное значение.